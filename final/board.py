...
class Hive(Board):
    ...
    def check_valid(state):
        ...
        occupied_positions = list(pieces.keys())
        visited = set()
        Hive.dfs(occupied_positions[0], occupied_positions, visited)
    ...
    def successor(self, state):
        ...
        for piece_name, piece in pieces.items():
            possible_moves = self.get_adjacent_spaces(piece.position)

            for new_pos in possible_moves:
                if self.is_valid_slide(piece.position, new_pos):
                    new_state = self.generate_new_state(state, piece_name, new_pos)
                    if self.check_valid(new_state):
                        successors[(piece_name, new_pos)] = new_state

        return successors

    def generate_new_state(self, state, piece_name, new_pos):
        board, pieces = state
        new_pieces = pieces.copy()
        new_pieces[piece_name] = new_pos  # Помести ја фигурата
